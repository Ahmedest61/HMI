...
#    data['load'] = load
	...
	flash(u'The user needs to check atleast one option','info')
        return render_template('fillForm.html')

    elif 'checkBurst' in _keys and 'checkNormalTime' in _keys:
	flash('The user cannot check burst and check normal time at once','error')
        return render_template('fillForm.html')

    elif 'checkBurst' in _keys and 'checkUserDefinedPackets' in _keys:
	flash('The user cannot check burst and user defined frame at once','error')
        return render_template('fillForm.html')

    elif 'checkUserDefinedPackets' in _keys and 'checkNormalTime' in _keys:
	flash('The user cannot check user defined frame and check normal time at once','error')
        return render_template('fillForm.html')
    ...
    if no_of_frameB<1:
	flash('Frames cannot compensate in the user defined burst parameters ','error')
        return render_template('fillForm.html')
    ...
    if no_of_frameN<1:
	flash('Frames cannot compensate in the user defined time parameters ','error')
        return render_template('fillForm.html')
